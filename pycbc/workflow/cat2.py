...
class PyCBCCalculateDqExecutable(Executable):
    ...
    def create_node(self, workflow, segments, cat2_file, flag):
        node = Node(self)
        ...
        node.add_opt('--analyzable-segments', segments)
...
def create_cat2_timeseries(workflow, segs, cat2_file, cat2_name, option_name,
    ...
    if not workflow.cp.has_option('workflow-coincidence', 'do-dq-fitting'):
        ...
        for ifo in workflow.ifos:
            ...
            for flag in flag_str:
                flag_tag = tags
                flag_tag.append(flag_str)
                raw_exe = PyCBCCalculateDqExecutable(workflow.cp,
                                                     'calculate_dq', ifos=ifo,
                                                     out_dir=output_dir,
                                                     tags=flag_tag)
                raw_node = raw_exe.create_node(workflow, segs[ifo], cat2_file, 
                                               flag_str)
                workflow += raw_node
